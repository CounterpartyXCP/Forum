...
# Definitions
    ...
    LUT
        A lookup table for addresses. Addresses are expressed as indexes into this table with the minimum amount of bits required for the amount of elements in it.

    RLE
        Run-length Encoding, a compression technique used to express multiple values grouped by type. In the case of this CIP, the type of the groups are the Counterparty assets, and the values are the indexes on the address LUT paired with amounts to be sent.
