...
The MPMA sends messages should by identified by the message type 0x02 inside a standard, CIP11 compliant, message envelope.
 ...
 * Changes needed to get MPMA onto testnet: (50% bounty)
   * CP Message ID allocation (`/lib/messages/send.py`).
   * Encoding of the MPMA transaction as a whole buffer inside CP code,
   ...
   transactions (a new file in `/lib/messages/versions/mpma.py`), this includes
   consensus validity detection of each send and modification of database with
   results of the MPMA.
   * Unit tests (vectors and tests added on `/test/fixtures/`)
 * Maintenance and bug testing for 3 months. (25% bounty)
 * Release to Mainnet. (25% bounty)
